@extends('layouts.app')

@section('content')

<h1 class="display-3 text-center fw-bolder">
      DATOS CURIOSOS
  </h1>
  <div class="card" style="padding-right: 90px; padding-left: 90px; background-color: rgb(240,240,240);">
    <div class="rounded d-block">
      <div class="modal-content card mb-3">
      <div class="row">
        <div class="col-6 p-3">
          <img src="sabias1.jpg" class="img-thumbnail" width="500" height="600">
        </div>
        <div class="col-6">
          <h1 class="display-6 text-center fw-bolder">
            DATOS CURIOSOS
        </h1>
        <div class="card bg-light p-3">
          <div class="text-center">
            <p class="fs-2 p-5">
              El disquete o diskette, un disco magnético portátil para almacenar datos, vio la luz en 1971 de la mano de IBM.
              El disquete ha quedado obsoleto precisamente por su limitada capacidad de almacenamiento
            </p> 
          </div>
        </div>
        <div class="text-center p-3">
          <a class="btn btn-warning fs-5 fw-bold" href="#" role="button">Siguiente dato</a>
      </div>
        </div>
       
      </div>
      </div>
    </div>

    </div>

  </div>

@endsection